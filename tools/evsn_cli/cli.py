...
DEFAULT_INTELLISENSE_CHECK_SOURCE = "evsn-runtime/can_hal/src/qnx/qnx_devctl_can_channel.cpp"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    intellisense = subparsers.add_parser(
        "intellisense",
        help="Set up native QNX/RTI IntelliSense and run a clangd smoke check.",
    )
    intellisense.add_argument("--dry-run", action="store_true", help="Print the setup plan without executing it.")
    intellisense.add_argument("--skip-check", action="store_true", help="Skip the clangd smoke check.")
    intellisense.add_argument(
        "--check-source",
        default=DEFAULT_INTELLISENSE_CHECK_SOURCE,
        help="Source file to check with clangd after setup.",
    )
    intellisense.add_argument("--build-type", default=DEFAULT_BUILD_TYPE, help="CMake build type for configure.")
...
def _intellisense_header_export_script() -> str:
    return textwrap.dedent(
        r"""
        set -euo pipefail
        ...
        dst=/workspace/project/.intellisense
        qnx_dst="${dst}/qnx800"
        rti_dst="${dst}/rti_connext_dds-7.3.1"
        gcc_include="${QNX_HOST}/usr/lib/gcc/aarch64-unknown-nto-qnx8.0.0/12.2.0/include"

        for required in \
          "${QNX_TARGET}/usr/include" \
          "${QNX_TARGET}/usr/include/sys/can_dcmd.h" \
          "${QNX_TARGET}/usr/include/c++/v1" \
          "${gcc_include}" \
          "${NDDSHOME}/include"; do
          [[ -e "${required}" ]] || {
            echo "missing builder path: ${required}" >&2
            exit 1
          }
        done

        rm -rf "${qnx_dst}" "${rti_dst}"
        mkdir -p \
          "${qnx_dst}/target/qnx/usr" \
          "${qnx_dst}/target/qnx/usr/local" \
          "${qnx_dst}/host/linux/x86_64/usr/lib/gcc/aarch64-unknown-nto-qnx8.0.0/12.2.0" \
          "${rti_dst}"

        cp -a "${QNX_TARGET}/usr/include" "${qnx_dst}/target/qnx/usr/"
        if [[ -d "${QNX_TARGET}/usr/local/include" ]]; then
          cp -a "${QNX_TARGET}/usr/local/include" "${qnx_dst}/target/qnx/usr/local/"
        fi
        cp -a "${gcc_include}" "${qnx_dst}/host/linux/x86_64/usr/lib/gcc/aarch64-unknown-nto-qnx8.0.0/12.2.0/"
        cp -a "${NDDSHOME}/include" "${rti_dst}/"

        test -f "${qnx_dst}/target/qnx/usr/include/sys/can_dcmd.h"
        test -f "${rti_dst}/include/ndds/hpp/rti/util/util.hpp"
        echo "Updated .intellisense QNX/RTI header cache"
        """
    ).strip()


def _intellisense_configure_script() -> str:
    return textwrap.dedent(
        r"""
        set -euo pipefail
        ...
        [[ "$(uname -m)" == "x86_64" ]] || {
          echo "Refusing QNX configure: builder container must run as x86_64." >&2
          exit 1
        }
        : "${QNX_HOST:?QNX_HOST is not set in the builder}"
        : "${QNX_TARGET:?QNX_TARGET is not set in the builder}"
        ...
        if [[ -n "${CONNEXTDDS_ARCH:-}" && "${CONNEXTDDS_ARCH}" == *QNX* ]]; then
          ...
          if [[ -n "${CONNEXTDDS_ARCH:-}" ]]; then
            echo "Ignoring non-QNX CONNEXTDDS_ARCH for QNX configure: ${CONNEXTDDS_ARCH}" >&2
          fi
          export CONNEXTDDS_ARCH="${qnx_rti_arches[0]}"
        elif [[ "${#qnx_rti_arches[@]}" -eq 0 ]]; then
          echo "The builder image has no RTI QNX target libraries under ${NDDSHOME}/lib." >&2
        ...
        [[ -f /workspace/project/cmake/qnx.toolchain.cmake ]] || {
          echo "Missing QNX toolchain file at /workspace/project/cmake/qnx.toolchain.cmake." >&2
          exit 1
        }
        ...
        test -s /workspace/project/build/compile_commands.json
        echo "Updated QNX build compile_commands.json"
        """
    ).strip()


def _local_intellisense_ignore_command(repo: Repo) -> Command:
    script = (
        "set -euo pipefail\n"
        "mkdir -p .git/info\n"
        "touch .git/info/exclude\n"
        "grep -qxF '.intellisense/' .git/info/exclude || printf '.intellisense/\\n' >> .git/info/exclude\n"
    )
    return Command(("bash", "-lc", script), cwd=repo.root)
...
def _intellisense_plan(
    ...
    *,
    ...
    commands = [
        _local_intellisense_ignore_command(repo),
        backend.ensure_builder_command(),
        backend.exec_bash_command(_intellisense_header_export_script()),
        backend.exec_bash_command(_intellisense_configure_script(), [build_type]),
        backend.exec_bash_command(_compile_commands_copy_script()),
        _python_rewrite_command(repo),
    ]
    if check_source is not None:
        commands.append(Command(("clangd", f"--check={check_source}"), cwd=repo.root))
    return commands
...
def _handle_intellisense(args: argparse.Namespace, repo: Repo, env: Mapping[str, str], stdout: TextIO) -> int:
    build_type = _validate_build_type(args.build_type)
    check_source = None if args.skip_check else args.check_source
    backend = _backend(repo, env)
    commands = _intellisense_plan(repo, backend, build_type=build_type, check_source=check_source)
    if args.dry_run:
        _print_plan(commands, stdout)
        return 0
    _prepare_docker_execution(repo, env)
    if check_source is not None and shutil.which("clangd") is None:
        raise EVSNError("clangd is required for the IntelliSense smoke check; pass --skip-check to omit it.")
    _run_plan(commands)
    return 0
...
def main(
    ...
    *,
    ...
    try:
        ...
        if args.command == "intellisense":
            return _handle_intellisense(args, repo, env, stdout)
