...
class DryRunPlanTests(unittest.TestCase):
    ...
    def test_intellisense_dry_run_plans_header_export_configure_rewrite_and_check(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = make_repo(Path(tmp), env_file=True)
            stdout = io.StringIO()

            code = cli.main(
                ["--repo-root", str(repo), "intellisense", "--dry-run"],
                stdout=stdout,
                stderr=io.StringIO(),
                env={},
                cwd=repo,
            ...
            self.assertEqual(0, code)
            output = stdout.getvalue()
            self.assertIn(".intellisense/", output)
            self.assertIn("Updated .intellisense QNX/RTI header cache", output)
            self.assertIn("-DCMAKE_EXPORT_COMPILE_COMMANDS=ON", output)
            self.assertIn("Updated out/qnx/compile_commands.container.json", output)
            self.assertIn("python3 -m evsn_cli.compile_commands", output)
            self.assertIn("clangd --check=evsn-runtime/can_hal/src/qnx/qnx_devctl_can_channel.cpp", output)

    def test_intellisense_skip_check_omits_clangd(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = make_repo(Path(tmp), env_file=True)
            stdout = io.StringIO()

            code = cli.main(
                ["--repo-root", str(repo), "intellisense", "--dry-run", "--skip-check"],
                stdout=stdout,
                stderr=io.StringIO(),
                env={},
                cwd=repo,
            ...
            self.assertEqual(0, code)
            self.assertNotIn("clangd --check", stdout.getvalue())
...
class CompileCommandRewriteTests(unittest.TestCase):
    def test_rewrite_compile_commands_replaces_container_paths_and_qcc_syntax(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("include_aarch64/machine/_types.h", rewritten)
            self.assertIn(str(repo.resolve() / "vcan-drivers" / "dev-can-linux"), rewritten)
            self.assertIn("--target=aarch64-unknown-nto", rewritten)
            self.assertIn("-D__LITTLEENDIAN__=1", rewritten)
            self.assertIn("-DQNX_LIBM_BUILTINS=1", rewritten)
